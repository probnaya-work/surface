...
export function requestMessage({ from, email, reference, receivedAt }) {
  return {
    ...
    text: [
      ...
      `REFERENCE  ${reference}`,
      '',
      'To establish access, from a clean checkout of probnaya-work/surface in access/:',
      '  npm ci',
      '  set ACCESS_ENV=production and DATABASE_URL (access_operator role,',
      '  sslmode=verify-full) at a hidden prompt; no application secrets are needed',
      '  npm run holders',
      `  npm run create-enrollment -- --new 'PROB–H–…' '${reference}'`,
      '',
      'Send the printed link to this address as a new message, not as a reply to',
      'this notification. The link is establishment authority: send it once, to',
      'this address only, and keep no other copy.',
    ].join('\n'),
  };
}
