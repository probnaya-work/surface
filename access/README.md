...
| `npm run approve-request -- 'R–XXXXXX'` | Normal request path. Prompts silently for the saved `access_operator` URL in production, checks the connected role, then asks for explicit approval. Atomically allocates the next four-digit `PROB–H` identifier and prints a ready-to-send message containing one establishment link. Refuses a reference that already produced a grant; never reissues. Optionally reads, without echo, the address the link will be sent to and stores only its keyed lookup for Observation ownership ([docs/observation-ownership.md](../docs/observation-ownership.md)); the address itself is never stored. |
...
| `npm run observation-owner -- register\|offer\|detach\|list …` | Private ownership of published Observations: registers a sender's keyed contact lookup (address read without echo, never an argument), offers an Observation to one verified holder, detaches an incorrect association without touching the public page, or lists states. See [docs/observation-ownership.md](../docs/observation-ownership.md). |
...
`002_holder_authority.sql` invalidates live ordinary and recovery sessions for non-active holders and installs the same invalidation for future suspension transitions; it does not delete credentials or recovery codes. `003_suspension_expires_grants.sql` extends that trigger so suspension also expires outstanding enrollment grants. `004_observation_ownership.sql` adds the private Observation ownership tables and a nullable `contact_lookup` on grants, with least-privilege grants for both roles when they exist.
