...
Each Observation carries a **trace** in its margin, on the index and on its own sheet: a small figure drawn at build time from its own text (`renderTrace` in `scripts/observations/render.js`). Every sentence is one graduation, as tall as the sentence is long. A paragraph or list item ends with a gap, a heading the sender wrote is a short mark below the line, and the longest sentence is PROBNAYA blue. It is decorative (`aria-hidden`), shows no numbers, and is identical for identical text. An Observation with no text has no trace.
...
## 11. The reading count

Each Observation shows, as the last line of its margin, how many times it has been read: `READ ONCE`, `READ 12 TIMES`. It appears on the piece's own sheet and beside it in the index, and nowhere else. Nothing is shown until there is a count, or when the store cannot be reached. The total across Observations is never shown (§6).

**What a reading is.** One browser, one Observation, once a day. On its own sheet a piece is read by being opened. In the index, where every piece is shown whole, it is read once its sheet has been at least half in view (or has filled half the screen, for a sheet taller than it) in a visible tab for three seconds. The day it was last counted is kept in that browser's `localStorage` (`probnaya:read:<number>`) and nowhere else. `js/observation-views.js` does this.

**What is sent and stored.** A reading is `POST /api/views` with the archival number and nothing else. No cookie, identifier, or address is set, sent, stored, or logged. `api/views.js` accepts it only from a page on this site (the `Origin` header), which keeps casual inflation out; it is not an authentication boundary. The store holds one integer per number, under `observation:views:<number>`. Counts are read with `GET /api/views?n=001,002`, cached at the edge for 30 seconds.

**The store** is one JSON file, `observations/views.json`, in a Vercel Blob store on the `surface` project (Vercel's own storage; no third-party service was taken on). It is read and written over the Blob REST API with `fetch`, so there is no client dependency, and it is read past the CDN copy so a reading taken a second ago is already in it. The handler reads `BLOB_READ_WRITE_TOKEN`, which Vercel sets when the store is connected; without it, pages show no count and readings are not taken. This is the section's only runtime store; the publishing workflow still uses none (§9), and nothing in it knows who read what.

**A reading can be lost.** A reading is read-modify-write on that one file, so two readings in the same instant can overwrite one another. This was accepted deliberately over running a database: the count is a reading count, not a ledger, and at this scale a collision is rare. If the section ever grows enough for that to matter, the store is one function in `api/views.js` (`blobStore`) and can be swapped for one with atomic increments without changing anything else.

**Readings before the counter.** Observations 001 and 002 were published before the counter existed. Their earlier readings were taken from Vercel Web Analytics by the same rule (unique visitors per day to the piece's own sheet, summed over days) and are added to the store's count as `BEFORE` in `api/views.js`. Views of the index were left out: they cannot be attributed to one piece. Nothing is back-filled for later Observations.

**Locally**, start the dev server with `VIEWS_DEV_MEMORY=1` for an in-memory store that forgets on restart. It is refused on Vercel. To count yourself again, remove the `probnaya:read:*` keys from the browser's local storage.
