...
| CSRF | `SameSite=Strict`; exact `Origin` on every POST; mandatory synchronizer token for ordinary authenticated mutations; JSON-only exact schemas. | XSS is same-origin and bypasses CSRF defenses. GET status still mutates CSRF/idle state and carries the independent review's low-severity cross-origin availability risk. |
...
| Sensitive logging | Central error mapping; log event type/outcome and opaque audit IDs only; never log assertions, challenges, cookies, enrollment tokens, recovery codes, raw credential IDs, or DB URLs. | Platform/request logs may still retain path, timing, and network metadata. |
